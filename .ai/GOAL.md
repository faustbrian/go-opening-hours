...
- `go-calendar` owns civil dates and zones; `go-temporal` owns time-of-day
  intervals; `go-clock` owns current time.
...
- Duplicate ranges, inverted ranges, precision loss, and day-boundary overflow
  fail predictably.
- Precision is explicit and compatible with `go-temporal/timeofday`.
...
- Helpers using current time require an injected `go-clock` capability; core
...
- `go-wire`, `go-validation`, and `go-config` adapters without dependency cycles.
- `go-calendar` integration for civil dates, weekdays, business calendars, and
  explicit timezone conversion.
- `go-temporal/timeofday` integration for daily and circular interval algebra.
- `go-clock` integration for `Now`, transition waiting examples, and tests.
- `go-localized` MAY provide localized labels while remaining optional.
