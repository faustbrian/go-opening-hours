...
policy. `Date` aliases the immutable `go-calendar` civil value, while
`LocalTime` remains distinct from both dates and instants. Schedule construction
uses `go-calendar/timezone` for bounded IANA identity loading; conversion to
...
`openinghourstemporal` accepts only `go-temporal/timeofday` mappings whose state,
...
Root aliases the narrow `go-clock` capability because current time belongs to
